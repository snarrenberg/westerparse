...
def displayFileSource(*args):
    ...
    if len(idxs) == 1:
        ...
        context.displaySourceAsPng(WPfile)
...
def evaluateSyntax(*args):
    ...
    if len(idxs) == 1:
        ...
        sys.stdout = temp_out 
        context.evaluateLines(WPfile, show='writeToPng', partSelection=None, partLineType=lt, report=True)
...
def evaluateCounterpoint(*args):
    ...
    if len(idxs) == 1:
        ...
        sys.stdout = temp_out
        # run the vl checker
        context.evaluateCounterpoint(WPfile, report=True)
        eval_report = sys.stdout.getvalue()
        temp_out.close()
        sys.stdout = sys.__stdout__
        reporttext.insert(INSERT, eval_report)
