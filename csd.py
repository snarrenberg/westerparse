...
class ConcreteScaleDegree:
    ...
    def __init__(self, p, scale):
        ...
        self.errors = []
        if scale.isConcrete == False:
            error = 'Cannot evaluate for the given scale.'
            ...
            if scale.type == "major":
                ...
                if scale.getScaleDegreeFromPitch(p) in {1, 2, 3, 4, 5, 6}:
                    ...
                    error = 'CSD Error: At least one of pitches in the line is not in the given scale of ' + scale.name + ': ' + p.nameWithOctave
                    self.errors.append(error)
        if dist >= 0:
            csd = 1 + dist
            dir = ""
        ...
        # I've created properties that assign the LaTeX codes that I use to obtain 
        # scale degree names: integers with a hat or bar above it
        if dist >= 0:
