...
class Analyzer:
    ...
    def addAnalysisData(self, score):
        ...
        # but may be accessed as .store[score.id]['ResultDict']
...
#         return allVLQs
    ...
    def getVLQs(self, score, partNum1, partNum2):
        ...
        part1_notesAndRests = part1.flat.notesAndRests
        part2_notesAndRests = part2.flat.notesAndRests
    ...
    # - startIndex is the first VLQ in the list to start with (0 is default)
    #   endIndex is the first VLQ in list not to search (length of VLQ list is default), 
    #   meaning default values are to search the entire vlqList
    ...
    def _identifyBasedOnNNoteLinearSegment(self, score, partNum, lengthLinearSegment, color, dictKey,
                                               testFunction, textFunction):

        self.addAnalysisData(score)
        if partNum is None:
            for partNumInner in range(len(score.parts)):
                self._identifyBasedOnNNoteLinearSegment(score, partNumInner, lengthLinearSegment, color, 
                                                       dictKey, testFunction, textFunction)
        else:
            nnlsList = self.getLinearSegments(score, partNum, lengthLinearSegment, classFilterList=(note.Note))
    ...
    def getParallelUnisons(self, score, partNum1=None, partNum2=None):
        ...
        self._identifyBasedOnVLQ(score, partNum1, partNum2, dictKey='parallelUnisons',
                            testFunction=testFunction)
        if self.store[sid]['ResultDict'] and 'parallelUnisons' in self.store[sid]['ResultDict']:
            return [tr.vlq for tr in self.store[sid]['ResultDict']['parallelUnisons']]
        else:
            return None
    ...
    def getHiddenFifths(self, score, partNum1=None, partNum2=None):
        ...
        self._identifyBasedOnVLQ(score, partNum1, partNum2, dictKey='hiddenFifths',
                            testFunction=testFunction)
        if self.store[sid]['ResultDict'] and 'hiddenFifths' in self.store[sid]['ResultDict']:
            return [tr.vlq for tr in self.store[sid]['ResultDict']['hiddenFifths']]
        else:
            return None
...
#         pass
   ...
   #-------------------------------------------------------------------------------
    # RS: Here are some functions for dealing with leaps of a fourth in the bass
    ...
    def identifyFourthLeapsInBass(self, score, color=None,
        ...
        textFunction = lambda nnls, pn: ("Leap of a perfect fourth in the bass in measure " + 
                                          str(nnls.objectList[0].measureNumber) +": " +
                                          " P4 from " +
                                          str(nnls.objectList[0].name) + " to " +
                                          str(nnls.objectList[1].name))
        self._identifyBasedOnNNoteLinearSegment(score, partNum, lengthLinearSegment, color, dictKey,
                                             testFunction, textFunction)
