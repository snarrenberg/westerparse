...
class VLQTheoryResult(TheoryResult):
    ...
    def color(self, color='red', noteList=(1, 2, 3, 4)):
        ...
        self.currentColor = color
        if 1 in noteList:
            self.vlq.v1n1.style.color = color
        if 2 in noteList:
            self.vlq.v1n2.style.color = color
        if 3 in noteList:
            self.vlq.v2n1.style.color = color
        if 4 in noteList:
            self.vlq.v2n2.style.color = color
...
class NNoteLinearSegmentTheoryResult(TheoryResult):

    _DOC_ATTR = {'nnls': '''The actual :class:`~music21.voiceLeading.NNoteLinearSegment`
                            object associated with this theory result object'''}

    def __init__(self, nnls):
        super().__init__()
        self.nnls = nnls

    def color(self, color ='red', noteList=(2,)):
        '''
        color all the notes in noteList. Default is to color all notes
        '''
        if noteList:
            for n in noteList:
                n.color = color
...
class VerticalityNTupletTheoryResult(TheoryResult):
    ...
    _DOC_ATTR = {'vsnt': '''The actual :class:`~music21.voiceLeading.VerticalityNTuplet`
                            object associated with this theory result object',
                            'partNumIdentified':'Storage location for
